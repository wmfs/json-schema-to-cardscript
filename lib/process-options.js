...
module.exports = function processOptions (jsonSchema, originalOptions) {
  ...
  if (options.fields === null) {
    options.fields = Object.keys(options.modelSchema.properties)
  }
  ...
  options.viewOnly = options.modelSchema && Object.entries(options.modelSchema.properties)
    .filter(([k, v]) => options.fields.includes(k))
    .map(([k, v]) => v.type)
    .filter(t => !t.endsWith('_output'))
    .length === 0
